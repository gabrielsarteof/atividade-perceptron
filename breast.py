...
import time
# Adicionado: Importações padrão do scikit-learn
...
# Removido: As funções manuais (split_data, MyStandardScaler) foram removidas
# para usar as versões do scikit-learn, padronizando o código.
...
X_train_full_A, X_test_A, y_train_full_A, y_test_A = train_test_split(X, y, test_size=0.3, random_state=42, stratify=y)
X_train_A, X_val_A, y_train_A, y_val_A = train_test_split(X_train_full_A, y_train_full_A, test_size=0.2, random_state=42, stratify=y_train_full_A)
...
plt.savefig('breast_chart.png', dpi=150, bbox_inches='tight', facecolor='white')
plt.tight_layout()
plt.show()
...
# Passo 2: Dividir em treino, validação e teste
X_train_full_B, X_test_B, y_train_full_B, y_test_B = train_test_split(X_full, y_full, test_size=0.3, random_state=42, stratify=y_full)
X_train_B, X_val_B, y_train_B, y_val_B = train_test_split(X_train_full_B, y_train_full_B, test_size=0.2, random_state=42, stratify=y_train_full_B)
...
O modelo com 2 features cometeu 38 Falsos Negativos
(câncer real diagnosticado como benigno, um erro
extremamente grave) e 1 Falso Positivo. Já o modelo
com 30 features reduziu drasticamente os erros, com
apenas 5 Falsos Negativos e 5 Falsos Positivos.
Isso prova que, para este problema, mais features
resultaram em um modelo não apenas mais preciso, mas
significativamente mais confiável e seguro para uma
aplicação médica, cumprindo o objetivo do exercício.
"""
