...
# Nossas funções manuais de pré-processamento
def split_data(X, y, test_size=0.3, random_state=None):
    ...
    # A CORREÇÃO ESTÁ AQUI: garantindo a ordem correta das variáveis
...
ppn_A = Perceptron(learning_rate=0.01, n_epochs=50)
ppn_A.fit(X_train_std_A, y_train)


# Passo 5: Avaliar o modelo
y_pred_A = ppn_A.predict(X_test_std_A)
accuracy_A = calculate_accuracy(y_test, y_pred_A)

print(f"\nAcurácia (2 features): {accuracy_A:.2%}")
print("\nRelatório de Classificação (2 features):")
print(classification_report(y_test, y_pred_A, target_names=cancer.target_names))
...
plt.sca(axes[0])
plot_decision_regions(X_train_std_A, y_train, classifier=ppn_A)
...
# Passo 2: Dividir em treino/teste
X_train_f, X_test_f, y_train_f, y_test_f = split_data(X_full, y_full, test_size=0.3, random_state=42)


# Passo 3: Normalizar os dados
scaler_B = MyStandardScaler()
scaler_B.fit(X_train_f)
X_train_std_B = scaler_B.transform(X_train_f)
X_test_std_B = scaler_B.transform(X_test_f)


# Passo 4: Treinar o perceptron
ppn_B = Perceptron(learning_rate=0.01, n_epochs=50)
ppn_B.fit(X_train_std_B, y_train_f)


# Passo 5: Avaliar o modelo
y_pred_B = ppn_B.predict(X_test_std_B)
accuracy_B = calculate_accuracy(y_test_f, y_pred_B)

print(f"\nAcurácia (30 features): {accuracy_B:.2%}")
print("\nRelatório de Classificação (30 features):")
print(classification_report(y_test_f, y_pred_B, target_names=cancer.target_names))
print("\nMatriz de Confusão (30 features):")
print(confusion_matrix(y_test_f, y_pred_B))
